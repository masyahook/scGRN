...
def netgraph_community_layout(
    ...
    if len(communities) < 2:
        warnings.warn(
            "Graph contains a single community. Unable to compute a community layout. "
            "Computing spring layout instead."
        )
        return nx.spring_layout(G, weight="importance")

    community_size = _get_community_sizes(node_to_community)
    community_centroids = _get_community_positions(
        G, node_to_community, community_scale, seed=seed
    )
    relative_node_positions = _get_node_positions(
        G, node_to_community, node_scale, seed=seed
    )
...
def _get_community_positions(
    ...
    :param G: NetworkX graph
    :param node_to_community: A dictionary where key is the node name and value is the community 
        number
    :param community_scale: A scale factor for positioning communities
    :param seed: A random seed
...
def _get_node_positions(
    ...
    for ci, nodes in communities.items():
        ...
        pos_subgraph = nx.spring_layout(
            subgraph, weight="importance", scale=node_scale, seed=seed
        )
...
def squeeze_graph(
    ...
    partition_sizes = {
        i: len([1 for node, k in partition.items() if k == i])
    ...
    normalized_partition_size = {
        i: (size // min_partition_size) for i, size in partition_sizes.items()
    }
    ...
    squeezed_partition = {
        i: (size * scale_factor) for i, size in normalized_partition_size.items()
    }
    ...
    for i, num_nodes in squeezed_partition.items():
        ...
        # Top nodes in the partition based on max number of inter-community edge (node_importance)
        top_nodes += list(
            dict(
                sorted(node_importance.items(), key=lambda x: x[1], reverse=True)[
                    : squeezed_partition[i]
                ]
            ).keys()
        )
    ...
    edges_to_keep = list(
        dict(
            sorted(
                {
                    (st, end): data["importance"]
                    for st, end, data in filtered_G.edges(data=True)
                }.items(),
                key=lambda x: x[1],
                reverse=True,
            )[:keep_num_edges]
    ...
    squeezed_partition = {
        node: i for node, i in filtered_partition.items() if node in squeezed_G.nodes()
    }
...
def process_communities(
    ...
    def _highlight_TFs(word, font_size, position, orientation, font_path, random_state):
        ...
        if word in lambert_TF_names or word in dorothea_TF_names:
            ...
            r, g, b, _ = plt.get_cmap("viridis")(font_size / 120)
    ...
    def _compute_community_info(i):
        ...
        central_genes_and_scores = {
            gene: all_partition_genes[i][gene]
        ...
        non_lambert_TFs = [
            f"{gene} (rank={k})"
            for k, gene in enumerate(central_genes_and_scores.keys(), start=1)
            if gene not in lambert_TF_names
        ]
        non_dorothea_TFs = [
            f"{gene} (rank={k})"
            for k, gene in enumerate(central_genes_and_scores.keys(), start=1)
            if gene not in dorothea_TF_names
        ]
        ...
        communities_i["sorted_central_genes_scores"] = "; ".join(
            f"{gene} (score={score:.2f})"
            for gene, score in central_genes_and_scores.items()
        )
        ...
        communities_i["whole_G_central_genes_scores"] = "; ".join(
            f"{gene} (score={score:.2f})"
            for gene, score in whole_G_central_genes.items()
        )

        # Filling information about newly found gene-gene links 
        # (based on absence in KEGG and Hallmark)
        top_cluster_links = set()

        iter_i = 0

        for st, end, edge_info in sorted(
            community_subgraph.edges(data=True),
            key=lambda t: t[2]["importance"],
            reverse=True,
        ):
            # If the current (reverse directed) link was not encountered previously..
            if (end, st) not in [
                (uniq_st, uniq_end) for uniq_st, uniq_end, _ in top_cluster_links
            ]:
        ...
        for anno_tag in ["KEGG", "hallmark"]:
            ...
            for st, end, imp in top_cluster_links:
                ...
                if len(st_anno_IDs.intersection(end_anno_IDs)) == 0 and (
                    ...
                    tmp_list.append(
                        f"{st} ({' & '.join(st_anno_IDs)}) <-> {end} ({' & '.join(end_anno_IDs)})"
                    )
        ...
        for tag, gene_func in gene_func_dbs.items():
            ...
            main_functions = list(
                dict.fromkeys(
                    [  # dropping duplicates, but preserving order
                        func
                        for gene in central_genes_and_scores.keys()
            ...
            gene_with_main_functions = [
                ",".join(
                    gene_func[gene_func == func]
                    .loc[lambda x: x.index.isin(genes)]
                    .index.to_list()
                )
            ...
            main_functions = [
                f">>> {func} <<<: {gene}"
                for gene, func in zip(gene_with_main_functions, main_functions)
            ]
            communities_i[f"main_functions_{tag}"] = "; ".join(
                main_functions
            )  # saving..

            # Saving functions corresponding to each gene
            central_functions_per_gene = [
                f">>> {gene} <<<: {' & '.join(gene_func[gene_func.index == gene].to_list())}"
                for gene in central_genes_and_scores.keys()
            ]
            communities_i[f"sorted_central_functions_{tag}"] = "; ".join(
                central_functions_per_gene
            )  # saving..
            ...
            freq_words = (
                dict(sorted(freq_words.items(), key=lambda x: x[1], reverse=True))
                if freq_words
                else {"no found function": 1}
            )  # dealing with no word case
            communities_i[f"most_frequent_function_words_{tag}"] = "; ".join(
                freq_words.keys()
            )  # saving
            ...
            other_functions = list(
                dict.fromkeys(
                    [  # dropping duplicates, but preserving order
                        func
                        for gene in genes
                        if gene not in central_genes_and_scores.keys()
                        for func in gene_func[gene_func.index == gene].to_list()
                        if func not in main_functions
                    ]
                )
            ...
            other_functions = [
                f">>> {func} <<<: {gene}"
                for gene, func in zip(genes_with_other_functions, other_functions)
            ]
            communities_i[f"other_functions_{tag}"] = "; ".join(
                other_functions
            )  # saving
        ...
        for k in range(num_partitions):  # t_sub:
            ...
            if i != k:
                ...
                # Subgraph contains central genes in community_i and all genes in comunity_k
                G_central_i_k = G.subgraph(
                    list(central_genes_and_scores.keys()) + genes_in_k
                )
                ...
                for st, end, edge_info in sorted(
                    ...
                    if (end, st) not in [
                        (uniq_st, uniq_end)
                        for uniq_st, uniq_end, _ in links_central_i_k
                    ] and (
                        (st in genes and end not in genes)
                        or (end in genes and st in genes)
                    ):
                ...
                # Getting out top links from the second subgraph
                for st, end, edge_info in sorted(
                    G_i_k.edges(data=True),
                    key=lambda t: t[2]["importance"],
                    reverse=True,
                ):
                    # If the current (reverse directed) link was not encountered previously..
                    if (end, st) not in [
                        (uniq_st, uniq_end) for uniq_st, uniq_end, _ in links_i_k
                    ] and (
                        (st in genes and end not in genes)
                        or (end in genes and st in genes)
                    ):
                ...
                communities_i[
                    f"top_links_scores_central_genes<->community_{k}"
                ] = "; ".join(
                    f"{st} <-> {end} (score={score:.2f})"
                    for st, end, score in links_central_i_k
                )
                communities_i[f"top_links_scores_with_community_{k}"] = "; ".join(
                    [
                        f"{st} <-> {end} (score={score:.2f})"
                        for st, end, score in links_i_k
                    ]
                )
    ...
    if pat is None or "all" in pat:
        ...
        if cell_type in ["all_data", "raw_data", "all"]:
            tag = "raw_data"
        # Specific cell type
        else:
    ...
    dorothea_TF_names = list(
        ...
        .loc[lambda x: x["confidence"].isin(["A", "B", "C"])]["tf"]
        .unique()
    )
    ...
    print(
        f"Loaded the graph: {colored('pat', 'green')}='{colored(pat, 'red')}', "
    ...
    if algo == "louvain":
        partition = community_louvain.best_partition(
            G.to_undirected(), weight="importance", random_state=seed
        )
    elif algo == "leiden":
        G_igraph = ig.Graph.from_networkx(G.to_undirected())
        la_partition = la.find_partition(
            G_igraph, la.ModularityVertexPartition, weights="importance", seed=seed
        )
        partition = {
            G_igraph.vs[node]["_nx_name"]: i
            for i, cluster_nodes in enumerate(la_partition)
    ...
    num_partitions = len(set(partition.values()))
    print(
        f'Number of partitions using {algo} algorithm: {colored(num_partitions, "cyan")}\n'
    )
    ...
    whole_G_central_genes = dict(
        sorted(
            betweenness_centrality_parallel(G, processes=num_workers, **kwargs).items(),
            key=lambda x: x[1],
            reverse=True,
        )[:limit_anno_until]
    )
    print(
        f'Computed the {"betweenness" if if_betweenness else "closeness"}'
        f' centrality for all genes in the graph\n'
    )

    ###### FINDING CENTRAL GENES AND FUNCTIONS IN EACH COMMUNITY USING FUNCTIONAL ANNOTATION ######
    ...
    gene_func_dbs = {
        tag: load_gene_func_db(tag, as_series=True) for tag in anno_db_tags
    }

    # Reversing partition dict -> {group_1: [gene_1, gene_2, ...], group_2: [gene_3, ...], ...}
    partition_genes_ = {}
    ...
    kwargs = (
        {"weight": "distance", "normalized": True}
        if if_betweenness
    ...
    for i, genes in t:
        ...
        central_gene_scores = {
            ...
            for k, gene in enumerate(gene_scores.keys())
            if k < limit_anno_until
        }
    ...
    squeezed_pos = netgraph_community_layout(
        squeezed_G, squeezed_partition, seed=seed
    )
    partition_coords = {}
    ...
    cmap = ListedColormap(
        sns.color_palette(cc.glasbey_bw, n_colors=num_partitions).as_hex()
    )
    ...
    for plot_type in ["genes"] + list(map(lambda x: f"func_{x}", anno_db_tags)):
        ...
        if plot_type == "genes":
            wordclouds = {
                ...
                .recolor(color_func=_highlight_TFs)
            ...
            word_counts = {
                ...
                for i, text in curr_partition_funcs.items()
            }
            word_counts = {
        ...
        ax.set_title(
            f'Found communities ({pat}, "all", {cell_type}), '
    ...
    result = Parallel(n_jobs=num_workers)(
        delayed(_compute_community_info)(i) for i in t
    )
    communities_df = pd.concat(result, axis=1).T.reindex(
        columns=[
            ...
            "other_functions_GO",
            "other_functions_KEGG",
            "other_functions_immunological",
            "other_functions_hallmark",
            "sorted_central_genes_scores",
            "sorted_central_functions_GO",
            "sorted_central_functions_KEGG",
            "sorted_central_functions_immunological",
            "sorted_central_functions_hallmark",
            "most_frequent_function_words_GO",
            "most_frequent_function_words_KEGG",
            "most_frequent_function_words_immunological",
            "most_frequent_function_words_hallmark",
            "all_sorted_genes",
        ]
        + [
            f"top_links_scores_central_genes<->community_{i}"
            for i in range(num_partitions)
        ]
