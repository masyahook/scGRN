...
def get_meta(data_home: str = _DATA_HOME, meta_file: str = _META_FILE) -> pd.DataFrame:
    """
    ...
    for patient_id in full_meta.index:
        ...
        for cell_type, cell_count in (
            cells_metadata["cell_type"].value_counts().iteritems()
        ):
...
def get_avail_sc_data(
    data_home: str = _DATA_HOME, meta_file: str = _META_FILE
) -> pd.DataFrame:
    """
    ...
    avail_sc_data = pd.DataFrame(
        ...
        + full_meta.columns[
            3:
        ].to_list(),  # cell-types, e.g. 'T_cells', 'Macrophage', 'all_data'
        index=["all_data"]
        + pat_types
        + full_meta.index.to_list(),  # patients, e.g. 'C51', 'C141', 'S', 'all_data'
    )
    ...
    for pat in avail_sc_data.index[:4]:
        ...
        if pat == "all_data":
            ...
            cell_types = ["all_data"] + full_meta[full_meta["group"] == pat].iloc[
                :, 3:
            ].loc[:, lambda x: ~x.isna().all()].columns.to_list()
        for d in cell_types:
    ...
    for net_type, data in avail_adj_lists.items():
        ...
        for pat in data.index[4:]:
            for d in avail_sc_data.loc[pat].dropna().index.to_list():
                d_fn = f"raw_data_{d}" if d != "all_data" else "raw_data"
                fn = os.path.join(
                    data_home, pat, "data", method, f"{d_fn}_{net_type_suffix}.tsv"
                )
                fn_pickled = os.path.join(
                    data_home,
                    pat,
                    "data",
                    method,
                ...
                filtered_fn_pickled = os.path.join(
                    data_home,
                    pat,
                    "data",
                    method,
...
def get_sc_data(
    ...
    # Loading patient-specific data
        ...
        fn = os.path.join(data_home, pat, "data", "Seurat", f"{tag}.tsv")
...
def get_num_cells(pat: str, cell_type: str, meta: pd.DataFrame) -> int:
    ...
    return (
        int(curr_meta[col_key])
        if isinstance(curr_meta[col_key], (int, float))
        else int(curr_meta[col_key].sum())
    )
...
def get_viper_mat(
    ...
    # Loading data that includes a specific patient type: 'control', 'moderate' or 'severe'
        ...
        fn = os.path.join(
            ...
            "regulon",
            "pickle",
            f"{regulon}_raw_data_{pat}_type.pickle",
        )
...
def get_adj_list(
    ...
    if pat is None or "all" in pat:
        ...
        fn = os.path.join(
            ...
            method,
            "pickle",
            f"raw_data_{net_suffix}{filter_suffix}.pickle",
        )
    ...
    # Loading patient-specific data
        ...
        fn = os.path.join(
            ...
            method,
            "pickle",
            f"{tag}_{net_suffix}{filter_suffix}.pickle",
        )
...
def get_nx_graph(
    ...
    if pat is None or "all" in pat:
        ...
        fn = os.path.join(
            ...
            method,
            "nx_graph",
            f"raw_data_{net_suffix}{filter_suffix}.gpickle",
        )
    ...
    # Loading patient-specific data
        ...
        fn = os.path.join(
            ...
            method,
            "nx_graph",
            f"{tag}_{net_suffix}{filter_suffix}.gpickle",
        )
...
def _compute_graph_stats(
    ...
    :param curr_ctype_: The cell type name of the data - could be either:
        e.g. 'Macrophage', 'T_cells' (the cell type identifier)
        e.g. 'all', 'all_data' (the aggregated data - include all cell types)
        e.g. 'raw_data_Macrophage', 'raw_data_T_cells', 'raw_data' (the data file identifier,
            e.g. 'raw_data_T_cells' corresponds to the same data as 'T_cells')
    ...
    :param _meta: The dataframe containing metadata about patients (cell count, patient types,
        file paths)
    ...
    try:
        ...
        if not nx.is_connected(full_G.to_undirected()):
            Gcc = sorted(
                nx.connected_components(full_G.to_undirected()), key=len, reverse=True
            )
        ...
        out = {
            ...
            "average_degree": np.mean(
                [degree for node, degree in full_G.degree(full_G.nodes())]
            ),
            "radius": G_igraph.radius(),
            "average_path_length": G_igraph.average_path_length(),
            "diameter": G_igraph.diameter(),
            "importances": [
                info["importance"] for st, end, info in full_G.edges(data=True)
            ],
            ...
            "median_rho": np.median(
                [info["rho"] for st, end, info in full_G.edges(data=True)]
            ),
...
def get_graph_stats(meta: pd.DataFrame, n_jobs: int, filtered: float = None) -> dict:
    ...
    :param meta: The dataframe containing metadata about patients (cell count, patient types,
        file paths)
    ...
    all_out = Parallel(n_jobs=n_jobs)(
        delayed(_compute_graph_stats)(_pat, _ctype, _ntype, meta, filtered)
        for _ntype in n_types
    ...
    for _n_type in n_types:
        graph_stats[_n_type]["importances"] = list(
            itertools.chain(*graph_stats[_n_type]["importances"])
        )
        graph_stats[_n_type]["rhos"] = list(
            itertools.chain(*graph_stats[_n_type]["rhos"])
        )
