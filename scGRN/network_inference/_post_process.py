"""Post processing of generated adjacency lists."""
import os
...
def post_process_adj_list(fn, q_thresh):
    """
    Post-process the adjacency list that is produced by GRN algorithm and save the Networkx GRN to
    pickle format.
    ...
    if "ctx" in fn:
        df = pd.read_csv(
            fn, sep="\t", index_col=[0, 1], header=[0, 1], skipinitialspace=True
        )
        all_df = pd.read_csv(os.path.join(curr_dir, fn.replace("ctx", "cor")), sep="\t")

        df[("Enrichment", "Context")] = df[("Enrichment", "Context")].apply(
            lambda s: eval(s)
        )
        df[("Enrichment", "TargetGenes")] = df[("Enrichment", "TargetGenes")].apply(
            lambda s: eval(s)
        )
        ...
        tf_target_info = (
            ...
            .droplevel(axis=1, level=0)["TargetGenes"]
            .map(set)  # transform each list into set
            .groupby("TF")
            .agg(
                lambda x: reduce(lambda a, b: a.union(b), x)
            )  # combine all targets per TF
        )
        for tf, target_info in tf_target_info.iteritems():
            tf_target_dict["TF"] += [tf for target_name, score in target_info]
            tf_target_dict["target"] += [
                target_name for target_name, score in target_info
            ]
            tf_target_dict["importance"] += [
                score for target_name, score in target_info
            ]
    ...
    nx.write_gpickle(
        filtered_graph, f"nx_graph/{short_fn}_filtered_{q_thresh_suffix}.gpickle"
    )
