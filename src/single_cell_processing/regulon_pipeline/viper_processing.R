...
for(i in 1:nrow(meta)){
  ...
  # Running viper
  cat("      - Running viper\n")
  run_on_cores <- max(detectCores() %/% 2, 0)
  ...
  write.table(sobj@assays$dorothea@data, file=paste0(sample_data_dir, "/dorothea_raw_data.tsv"), sep="\t", row.names=T, col.names=colnames(sobj@assays$dorothea@data), quote=F)
  ...
  for (i in 1:length(cell_types))
  {
    ...
    if(opt$serialize==T) save(cell_type_sobj, file=paste0(sample_data_dir, sprintf("/dorothea_seurat_object_%s.RData", type)))
    write.table(cell_type_sobj@assays$dorothea@data, file=paste0(sample_data_dir, sprintf("/dorothea_raw_data_%s.tsv", type)), sep="\t", row.names=T, col.names=colnames(cell_type_sobj@assays$dorothea@data), quote=F)
  }
}
